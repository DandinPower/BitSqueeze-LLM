...
static inline double fp64_from_bits(unsigned long long w) {
#if defined(__OPENCL_VERSION__)
    return as_double(w);
#elif defined(__CUDA_ARCH__)
...
static inline unsigned long long fp64_to_bits(double f) {
#if defined(__OPENCL_VERSION__)
    return as_ulong(f);
#elif defined(__CUDA_ARCH__)
    return (unsigned long long)__double_as_longlong(f);
#elif defined(__INTEL_COMPILER) || defined(_MSC_VER) && (_MSC_VER >= 1932) && (defined(_M_IX86) || defined(_M_X64))
    return _castf64_u64(f);
#elif defined(_MSC_VER) && !defined(__clang__) && (defined(_M_ARM) || defined(_M_ARM64))
    return (unsigned long long)_CopyInt64FromDouble(f);
#else
    union {
        double as_value;
        unsigned long long as_bits;
