...
int main(void) {
    ...
    float **inputs = gen_random_float_arrays(X, N, MINV, MAXV, 12345);
    if (!inputs) return EXIT_FAILURE;

    for (unsigned long long k = 0; k < X; ++k) {
        quantization_buffer_t *buf = NULL;
        double t0 = get_time_ms();
        if (quantization_compress(inputs[k], N, Q8_0, &buf) || !buf) return EXIT_FAILURE;
        ...
        printf("[array %llu] Q8_0: size=%.3f KB, B/W=%.5f, MAE=%.6f, MSE=%.6f, MaxAbs=%.6f\n",
