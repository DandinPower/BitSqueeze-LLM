...
struct DeviceBuffers {
    ...
    uint8_t *d_block_scales = nullptr;
...
int run_case(const float *input, unsigned long long count) {
    ...
    try {
        ...
        check_cuda(cudaMalloc(&buffers.d_dq_scale, sizeof(float)), "cudaMalloc d_dq_scale failed");
        check_cuda(cudaMalloc(&buffers.d_block_scales, static_cast<std::size_t>(num_blocks) * sizeof(uint8_t)),
        ...
        if (header.method != NF4_DQ || header.num_elements != count) {
            throw std::runtime_error("unexpected quantization_cuda packed header");
        }

        check_cuda(cudaMemset(buffers.d_dq_scale, 0, sizeof(float)), "cudaMemset d_dq_scale failed");
        check_cuda(cudaMemset(buffers.d_block_scales, 0, static_cast<std::size_t>(num_blocks) * sizeof(uint8_t)),
        ...
        if (quantization_compress(input, count, NF4_DQ, &legacy) != 0 || legacy == nullptr) {
            throw std::runtime_error("legacy NF4_DQ compress failed");
        }
        std::vector<float> legacy_deq(static_cast<std::size_t>(count), 0.0f);
        if (quantization_decompress(legacy, legacy_deq.data(), count) != 0) {
            throw std::runtime_error("legacy NF4_DQ decompress failed");
        }

        for (unsigned long long i = 0; i < count; ++i) {
            if (cuda_deq[static_cast<std::size_t>(i)] != legacy_deq[static_cast<std::size_t>(i)]) {
                std::fprintf(stderr, "NF4_DQ CUDA mismatch at %llu\n", i);
                return 1;
            }
        }
        ...
        std::printf(
            "NF4_DQ CUDA roundtrip: packed=%llu bytes, MAE=%.6f, MSE=%.6f, MaxAbs=%.6f\n",
        ...
        std::fprintf(stderr, "NF4_DQ CUDA test failed: %s\n", e.what());
        quantization_free(legacy);
        quantization_cuda_release(&ctx);
        return 1;
    }
...
int main(void) {
    ...
    float **inputs = gen_random_float_arrays(2, count, -10.0f, 10.0f, 54321);
    if (inputs == nullptr) {
        return EXIT_FAILURE;
    }
